...
function updateImageSizeStyleTag(editor, position) {
    const getPropertyInsiderStyleTag = (editor) => {
        ...
        const currentNode = util_1.getFlatNode(rootNode, offset, true);
        if (currentNode && currentNode.name === 'style'
            ...
            const innerNode = util_1.getFlatNode(innerRootNode, offset, true);
...
function getImageHTMLNode(editor, position) {
    ...
    const rootNode = parseDocument_1.getRootNode(document, true);
    const offset = document.offsetAt(position);
    const node = util_1.getFlatNode(rootNode, offset, true);
...
function updateHTMLTag(editor, node, width, height) {
    ...
    if (!widthAttr) {
        ...
        edits.push(new vscode_1.TextEdit(util_1.offsetRangeToVsRange(document, widthAttr.value.start, widthAttr.value.end), String(width)));
    }
    if (!heightAttr) {
        textToAdd += ` height=${quote}${height}${quote}`;
    }
    else {
        edits.push(new vscode_1.TextEdit(util_1.offsetRangeToVsRange(document, heightAttr.value.start, heightAttr.value.end), String(height)));
    }
    if (textToAdd) {
        edits.push(new vscode_1.TextEdit(util_1.offsetRangeToVsRange(document, endOfAttributes, endOfAttributes), textToAdd));
    }
    return edits;
}
/**
...
function updateCSSNode(editor, srcProp, width, height) {
    ...
    if (!srcProp.terminatorToken) {
        edits.push(new vscode_1.TextEdit(util_1.offsetRangeToVsRange(document, srcProp.end, srcProp.end), ';'));
    }
    let textToAdd = '';
    if (!widthProp) {
        ...
        edits.push(new vscode_1.TextEdit(util_1.offsetRangeToVsRange(document, widthProp.valueToken.start, widthProp.valueToken.end), `${width}px`));
    }
    if (!heightProp) {
        textToAdd += `${before}height${separator}${height}px;`;
    }
    else {
        edits.push(new vscode_1.TextEdit(util_1.offsetRangeToVsRange(document, heightProp.valueToken.start, heightProp.valueToken.end), `${height}px`));
    }
    if (textToAdd) {
        edits.push(new vscode_1.TextEdit(util_1.offsetRangeToVsRange(document, srcProp.end, srcProp.end), textToAdd));
    }
    return edits;
}
/**
