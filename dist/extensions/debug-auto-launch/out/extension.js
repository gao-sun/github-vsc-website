...
const TEXT_STATUSBAR_LABEL = {
    ["disabled" /* Disabled */]: localize('status.text.auto.attach.disabled', 'Auto Attach: Disabled'),
    ["always" /* Always */]: localize('status.text.auto.attach.always', 'Auto Attach: Always'),
    ["smart" /* Smart */]: localize('status.text.auto.attach.smart', 'Auto Attach: Smart'),
    ["onlyWithFlag" /* OnlyWithFlag */]: localize('status.text.auto.attach.withFlag', 'Auto Attach: With Flag'),
};
const TEXT_STATE_LABEL = {
    ["disabled" /* Disabled */]: localize('debug.javascript.autoAttach.disabled.label', 'Disabled'),
    ["always" /* Always */]: localize('debug.javascript.autoAttach.always.label', 'Always'),
    ["smart" /* Smart */]: localize('debug.javascript.autoAttach.smart.label', 'Smart'),
    ["onlyWithFlag" /* OnlyWithFlag */]: localize('debug.javascript.autoAttach.onlyWithFlag.label', 'Only With Flag'),
};
const TEXT_STATE_DESCRIPTION = {
    ["disabled" /* Disabled */]: localize('debug.javascript.autoAttach.disabled.description', 'Auto attach is disabled and not shown in status bar'),
    ["always" /* Always */]: localize('debug.javascript.autoAttach.always.description', 'Auto attach to every Node.js process launched in the terminal'),
    ["smart" /* Smart */]: localize('debug.javascript.autoAttach.smart.description', "Auto attach when running scripts that aren't in a node_modules folder"),
    ["onlyWithFlag" /* OnlyWithFlag */]: localize('debug.javascript.autoAttach.onlyWithFlag.description', 'Only auto attach when the `--inspect` flag is given'),
};
...
async function toggleAutoAttachSetting(context, scope) {
    ...
    const items = ["always" /* Always */, "smart" /* Smart */, "onlyWithFlag" /* OnlyWithFlag */, "disabled" /* Disabled */].map(state => ({
        state,
...
function readCurrentState() {
    var _a;
    const section = vscode.workspace.getConfiguration(SETTING_SECTION);
    return (_a = section.get(SETTING_STATE)) !== null && _a !== void 0 ? _a : "disabled" /* Disabled */;
}
 ...
 */
const transitions = {
    async ["disabled" /* Disabled */](context) {
        await clearJsDebugAttachState(context);
    },
    async ["onlyWithFlag" /* OnlyWithFlag */](context) {
        await createAttachServer(context);
    },
    async ["smart" /* Smart */](context) {
        await createAttachServer(context);
    },
    async ["always" /* Always */](context) {
        await createAttachServer(context);
    },
};
/**
...
function updateStatusBar(context, state, busy = false) {
    if (state === "disabled" /* Disabled */ && !busy) {
...
async function getIpcAddress(context) {
    ...
    if ((cachedIpc === null || cachedIpc === void 0 ? void 0 : cachedIpc.jsDebugPath) === jsDebugPath && (cachedIpc === null || cachedIpc === void 0 ? void 0 : cachedIpc.settingsValue) === settingsValue) {
        return cachedIpc.ipcAddress;
    }
    const result = await vscode.commands.executeCommand('extension.js-debug.setAutoAttachVariables', cachedIpc === null || cachedIpc === void 0 ? void 0 : cachedIpc.ipcAddress);
    if (!result) {
        return;
    }
