...
class Repository {
    ...
    async checkIfMaybeRebased(currentBranch) {
        ...
        const maybeRebased = await this.run("Log" /* Log */, async () => {
            try {
                const result = await this.repository.run(['log', '--oneline', '--cherry', `${currentBranch !== null && currentBranch !== void 0 ? currentBranch : ''}...${currentBranch !== null && currentBranch !== void 0 ? currentBranch : ''}@{upstream}`, '--']);
                if (result.exitCode) {
                    return false;
                }
